...
public:
  ...
  ~ShaderBase() { glDeleteProgram(program); }

  static std::string readGLSLFile(std::string fileName,
                                  int32_t &size); // Read File
  int addProgram(std::string fileName,
                 GLenum shaderType);                 // General Shader Addition
  ...
  static void error(GLuint s, bool t,
                    std::string fileName); // Get Compile/Link Error

  static std::unordered_map<std::string, GLuint>
      sbpi; // Shader Binding Point Index
  ...
  void interface(
      std::vector<std::string> names); // Add a list of buffers to interface
...
std::unordered_map<std::string, GLuint>
    ShaderBase::sbpi; // Shader Binding Point Index
  ...
  if (stream.is_open()) {
    ...
    while (std::getline(stream, line)) {
      if (line.substr(0, 9) == "#include ") {
        int includesize = 0;
        buffer << readGLSLFile((local_dir / line.substr(9)).string(),
                               includesize);
      } else
        buffer << line;
    }
...
void ShaderBase::error(GLuint s, bool t, std::string fileName) {
  int m;
  if (t)
    glGetShaderiv(s, GL_INFO_LOG_LENGTH, &m);
  else
    glGetProgramiv(s, GL_INFO_LOG_LENGTH, &m);
  char *l = new char[m];
  if (t)
    glGetShaderInfoLog(s, m, &m, l);
  else
    glGetProgramInfoLog(s, m, &m, l);
...
void ShaderBase::ssbo(std::string name) {
  if (sbpi.find(name) != sbpi.end())
    return; // Named Binding Point Exists
  sbpi[name] = sbpi.size();
}
...
void ShaderBase::interface(std::string name) {
  ...
  glShaderStorageBlockBinding(
      program,
      glGetProgramResourceIndex(program, GL_SHADER_STORAGE_BLOCK, name.c_str()),
      sbpi[name]);
}
...
template <typename T> void ShaderBase::bind(std::string name, Buffer *buf) {
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, buf->index);
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, sbpi[name], buf->index);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, 0);
}

template <typename T> void ShaderBase::uniform(std::string name, T u) {
  std::cout << "Error: Data type not recognized for uniform " << name << "."
            << std::endl;
}
...
template <>
void ShaderBase::uniform(
    std::string name, const double u) { // GLSL Intrinsically Single Precision
...
void ShaderBase::uniform(std::string name, const std::vector<glm::mat4> u) {
  glUniformMatrix4fv(glGetUniformLocation(program, name.c_str()), u.size(),
                     GL_FALSE, &u[0][0][0]);
}

template <typename T> void ShaderBase::texture(std::string name, const T &t) {
...
public:
  Compute(std::string shader)
      : ShaderBase(), computeShader(addProgram(shader, GL_COMPUTE_SHADER)) {
    link(shader);
  }
  ...
  ~Compute() { glDeleteShader(computeShader); }
  ...
  static void limits() {
    ...
    std::cout << "Max. SSBO: " << getInt(GL_MAX_SHADER_STORAGE_BUFFER_BINDINGS)
              << std::endl;
    std::cout << "Max. SSBO Block-Size: "
              << getInt(GL_MAX_SHADER_STORAGE_BLOCK_SIZE) << std::endl;
    std::cout << "Max. Compute Shader Storage Blocks: "
              << getInt(GL_MAX_COMPUTE_SHADER_STORAGE_BLOCKS) << std::endl;
    std::cout << "Max. Shared Storage Size: "
              << getInt(GL_MAX_COMPUTE_SHARED_MEMORY_SIZE) << std::endl;
...
#endif // TINYENGINE_UTILITY_SHADER
