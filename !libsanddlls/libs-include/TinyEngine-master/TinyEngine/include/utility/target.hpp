...
public:
  ...
  template <typename T>
  void bind(T &t,
            GLenum attach = GL_COLOR_ATTACHMENT0) { // Bind a texture to the FBO
  ...
  void sample(T *m, glm::vec2 p, glm::vec2 d,
              GLenum ATTACH = GL_COLOR_BUFFER_BIT, GLenum FORMAT = GL_RGBA) {
    if (d.x <= 0 || d.y <= 0 || p.x + d.x > WIDTH || p.y + d.y > HEIGHT)
      return;
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
    glReadBuffer(ATTACH);
...
public:
  ...
  Billboard(int _W, int _H) : texture(_W, _H), depth(_W, _H), Target(_W, _H) {
  ...
  Billboard(SDL_Surface *s)
      : // Render target preloaded with an image
        Billboard(s->w, s->h) {
    texture.raw(s); // Construct the texture from raw surface data
    bind(texture,
         GL_COLOR_ATTACHMENT0); // Bind it to the billboard as color texture
  }
};
...
public:
  ...
  Cubemap(int _W, int _H) : texture(_W, _H), depth(_W, _H), Target(_W, _H) {
    texture.setup({GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE});
    bind(texture, GL_COLOR_ATTACHMENT0);
    depth.setup({GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT, GL_UNSIGNED_BYTE});
    bind(depth, GL_DEPTH_ATTACHMENT);
