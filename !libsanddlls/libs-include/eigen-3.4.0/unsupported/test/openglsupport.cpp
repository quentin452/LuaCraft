...
GLint createProgram(const char *vtx, const char *frg,
...
GLint createProgram(const std::string &vtx, const std::string &frg,
                    bool print_errors = true) {
...
void find_and_replace(std::string &str, const std::string &find,
...
void openglsupport_test_loop() {
  ...
  std::cerr << "GLSL version: " << glGetString(GL_SHADING_LANGUAGE_VERSION)
            << std::endl;
  // Parse version from string since GL_MAJOR_VERSION is only supported in
  // GL 3.0+. Version string guaranteed to be <major>.<minor><vender extension>.
  GLint gl_major_version = gl_version_string[0] - '0';
  GLint gl_minor_version = gl_version_string[2] - '0';
  bool legacy_gl =
      gl_major_version < 3 || (gl_major_version == 3 && gl_minor_version < 2);
  ...
  if (legacy_gl) {
    ...
    // translations (2D and 3D)
    {
      ...
      VERIFY_MATRIX(glTranslate(vf2),
                    Projective3f(Translation3f(vf23)).matrix());
      Vector2d vd2;
      vd2.setRandom();
      Vector3d vd23;
      vd23 << vd2, 0;
      VERIFY_MATRIX(glTranslate(vd2),
                    Projective3d(Translation3d(vd23)).matrix());

      Vector3f vf3;
      vf3.setRandom();
      VERIFY_MATRIX(glTranslate(vf3),
                    Projective3f(Translation3f(vf3)).matrix());
      Vector3d vd3;
      vd3.setRandom();
      VERIFY_MATRIX(glTranslate(vd3),
                    Projective3d(Translation3d(vd3)).matrix());
  ...
  {
    // vertex shader.
    std::string vtx = format("${GLSL_VERSION}\n"
                             "void main(void) {\n"
    ...
    if (GLEW_VERSION_2_0 && GL_VERSION_2_0) {
      std::string frg = format("${GLSL_VERSION}\n"
                               ...
                               "vec4(v2f[0]+v3f[0]+v4f[0])+vec4(v2i[0]+v3i[0]+"
                               "v4i[0])+vec4(m2f[0][0]+m3f[0][0]+m4f[0][0]);\n"
                               "}\n",
                               glsl_vars, glsl_vals);
    ...
    if (GLEW_VERSION_2_1 && GL_VERSION_2_1 &&
      ...
      std::string frg = format("${GLSL_VERSION}\n"
                               ...
                               "void main(void) {\n"
                               "  ${FRAG_OUTPUT_VARIABLE} = "
                               "vec4(m23f[0][0]+m32f[0][0]+m24f[0][0]+m42f[0]["
                               "0]+m34f[0][0]+m43f[0][0]);\n"
                               "}\n",
                               glsl_vars, glsl_vals);
