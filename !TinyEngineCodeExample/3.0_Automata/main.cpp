...
int main(int argc, char *args[]) {
  ...
  Tiny::window("GPU Accelerated Cellular Automata Example", 1200,
               800); // Open Window
  Tiny::event.handler = [&]() {
  ...
  Shader shader({"shader/billboard.vs", "shader/billboard.fs"},
                {"in_Quad", "in_Tex"});
  Shader automata({"shader/automata.vs", "shader/automata.fs"},
                  {"in_Quad", "in_Tex"});
  ...
  Tiny::view.pipeline = [&]() {
    if (!paused) {
      if (flip) {
        ...
        flat.render(); // Render target texture to itself using primitive and
                       // automata shader
      } else {
    ...
    if (flip)
      shader.texture("imageTexture", textureA);
    else
      shader.texture("imageTexture", textureB);
