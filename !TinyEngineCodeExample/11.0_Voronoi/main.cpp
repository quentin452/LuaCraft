...
#include "../../TinyEngine/include/helpers/color.hpp"
...
Possible OpenGL Implementations (2 Types):
        ...
        (B) Depth Test Clustering:
                -> Instanced render with centroids, distance is depth, color
computed in shader

        Here I implemented the second version.

        Additional Optimizations:
                -> Don't draw entire quad when points are placed using poisson
disc, only a small quad This reduces fragment waste because we can assume cones
of a certain size. 2-3 Orders of Magnitude Speed Boost
                -> Don't pass a matrix, just the centroid. Scale in the vertex
shader.
        ...
        Applications:
                ...
                -> Dynamic Run-Time Mosaic Effects with Particles (e.g. Lava-,
Cloud-Flow)
...
int main(int argc, char *args[]) {
  ...
  // Utility Classes
  Square2D flat;
  Shader voronoi({"shader/voronoi.vs", "shader/voronoi.fs"},
                 {"in_Quad", "in_Tex", "in_Centroid"});

  Billboard billboard(_SIZE_, _SIZE_);
  Shader billboardshader({"shader/billboard.vs", "shader/billboard.fs"},
                         {"in_Quad", "in_Tex"});

  // Filter Effects
  Shader bubble({"shader/bubble.vs", "shader/bubble.fs"},
                {"in_Quad", "in_Tex", "in_Centroid"}, {"centroids"});
  Shader mosaic({"shader/mosaic.vs", "shader/mosaic.fs"},
                {"in_Quad", "in_Tex", "in_Centroid"}, {"centroids"});
  ...
  int n = 0;
  float us =
      0.0; // Rolling average execution time calculation in microseconds (us)
  ...
  Tiny::view.pipeline = [&]() {
    ...
    std::cout << "Average Execution Time: " << us
              << std::endl; // Comment out for less spam
