...
float uniform(float a, float b) { return dis(gen) * (a - b) + b; }

// Single point at position towards p with radius R
bool disc(std::vector<glm::vec2> &set, float R, glm::vec2 p, glm::vec2 a,
          glm::vec2 b) {
  ...
  while (tries > 0) {
    ...
    if (glm::any(glm::lessThan(npos, glm::vec2(a))) ||
        glm::any(glm::greaterThanEqual(npos, glm::vec2(b))))
      continue;
...
// Those functions could probably be improved. Not sure how right now.
// Recursiveness?
} // namespace sample
#endif // FILENAME_H