...
#include "../../TinyEngine/include/helpers/math.hpp"
...
int main(int argc, char *args[]) {
  ...
  Shader particledepth({"shader/particledepth.vs", "shader/particledepth.fs"},
                       {"in_Quad", "in_Tex", "in_Model"});

  Target shadow(1600, 1600);
  Texture depthtex(1600, 1600,
  ...
  Tiny::view.pipeline = [&]() { // Setup Drawing Pipeline
    ...
    if (drawwire || drawtree) {
      ...
      defaultShader.uniform(
          "drawcolor", glm::vec4(backcolor[0], backcolor[1], backcolor[2], 1));
      ...
      if (drawtree) {
        defaultShader.uniform(
            "drawcolor",
            glm::vec4(treecolor[0], treecolor[1], treecolor[2], treeopacity));
        defaultShader.uniform("wireframe", false);
        treemesh.render(GL_TRIANGLES);
      }

      if (drawwire) {
        defaultShader.uniform("drawcolor", glm::vec4(wirecolor[0], wirecolor[1],
                                                     wirecolor[2], 1.0));
    ...
    if (drawleaf) {
      ...
      particleShader.uniform("ff", glm::rotate(glm::mat4(1.0f),
                                               glm::radians(180 - cam::rot),
                                               glm::vec3(0, 1, 0)));
      particleShader.uniform("leafcolor", glm::vec4(leafcolor[0], leafcolor[1],
                                                    leafcolor[2], leafopacity));
