...
int main(int argc, char *args[]) {
  ...
  for (size_t i = 0; i < RES; i++)
    for (size_t j = 0; j < RES; j++) {
      vertices[0 * RES * RES + i * RES + j] =
          glm::vec4(i, j, (float)(rand() % 101) / 100.0f - 0.5, 1);
  ...
  for (size_t i = 0; i < RES - 1; i++)
    for (size_t j = 0; j < RES - 1; j++) {
      ...
      indices.emplace_back((i + 1) * RES + j, (i)*RES + j + 1,
                           (i + 1) * RES + j + 1, 1);
      indices.emplace_back((i)*RES + j, (i + 1) * RES + j + 1,
                           (i + 1) * RES + j, 1);
    }
