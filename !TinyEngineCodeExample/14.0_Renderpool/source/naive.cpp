    ...
    for (int i = 0; i < 5; i++)
      for (int j = 0; j < 5; j++)
        for (int k = 0; k < 5; k++) {
          ...
          chunkmesh::greedy(&chunks.back(), indices, positions, normals,
                            colors);
      ...
      [&]() { /* ... */
              ...
              for (int i = 0; i < chunks.size(); i++) {
                ...
                t += timer::benchmark<std::chrono::microseconds>(
                    [&]() {
                      chunkmesh::greedy(&chunks[i], models[i]->buffers["index"],
                                        models[i]->buffers["in_Position"],
                                        models[i]->buffers["in_Normal"],
                                        models[i]->buffers["in_Color"]);
