...
Idea: Use multi indirect rendering, combined with a memory pool, to render
multiple different objects with the same vertex format extremely quickly and
dynamically.

For multiple objects, we store vertices in an interleaved format in a single
VBO. All vertices for these objects have the same format.

The VBO is persistently mapped so that we can utilize the pointer in a memory
pool to distribute vertices to object constructors as they are needed.
...
struct Vertex {
  ...
  static void format(int vbo) {
    ...
    glBindVertexBuffer(0, vbo, offsetof(Vertex, position),
                       sizeof(Vertex)); // Internal Offset vs. Full Offset
...
struct DAIC {
  ...
  uint *index = nullptr; // Index Pointer
...
#ifndef Vertexpool_H
#define Vertexpool_H
template <typename T> class Vertexpool {
private:
...
public:
  ...
  void reserve(const int k, const int n) {
    ...
    N = n;
    M = n;
    const GLbitfield flag =
        GL_MAP_WRITE_BIT | GL_MAP_PERSISTENT_BIT | GL_MAP_COHERENT_BIT;

    glBindBuffer(GL_ARRAY_BUFFER, vbo);
    glBufferStorage(GL_ARRAY_BUFFER, N * K * sizeof(T), NULL, flag);
    startPtr =
        (T *)glMapBufferRange(GL_ARRAY_BUFFER, 0, N * K * sizeof(T), flag);
    MAXSIZE = N * K;

    for (int i = 0; i < N; i++)
      freeList.push_front(startPtr + i * K);
  }
  ...
  uint *section(const int size, const int group = 0, vec3 pos = vec3(0)) {
    ...
    const int base = (freeList.back() - startPtr);
    indirect.emplace_back(size, 1, first, base, new uint(indirect.size()),
                          group);
    freeList.pop_back();
  ...
  void unsection(uint *index) {
    ...
    if (index == NULL) {
      std::cout << "Vertexpool Error: Can't Unsection - Index is NULL"
                << std::endl;
      return;
    }
    ...
    for (int k = indirect[*index].baseVert; k < K; k++)
      (startPtr + k)->~T();
    freeList.push_front(startPtr + indirect[*index].baseVert);
  ...
  template <typename... Args> void fill(uint *ind, int k, Args &&...args) {

    T *place = startPtr + indirect[*ind].baseVert + k;
    if (size_t(place - startPtr) > MAXSIZE)
      std::cout << "Vertexpool Error: Out-Of-Bounds Write" << std::endl;

    try {
      new (place) T(forward<Args>(args)...);
    } catch (...) {
      throw;
    }
  }
  ...
  */
  ...
  void mask(F function, Args &&...args) {
    ...
    while (M <= J) {
      ...
      while (function(indirect[M], args...) && M < J)
        M++;
      while (!function(indirect[J], args...) && M < J)
        J--;
  ...
  void order(F function, Args &&...args) {
    ...
    sort(indirect.begin(), indirect.begin() + M,
         [&](const DAIC &a, const DAIC &b) { return function(a, b, args...); });
  ...
  */
  ...
  void update() {
    ...
    glBufferData(GL_DRAW_INDIRECT_BUFFER, indirect.size() * sizeof(DAIC),
                 &indirect[0], GL_DYNAMIC_DRAW);
  }
  ...
  */
  ...
  void render(const GLenum mode = GL_TRIANGLES, size_t first = 0,
              size_t length = 0) {
    ...
    glMultiDrawElementsIndirect(mode, GL_UNSIGNED_INT,
                                (void *)(first * (sizeof(DAIC))), length,
                                sizeof(DAIC));
  }
};
#endif // Vertexpool_H