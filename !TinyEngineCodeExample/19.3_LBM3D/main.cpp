...
int main(int argc, char *args[]) {
  ...
  Tiny::window("Lattice Boltzmann Method 3D, Terrain BC", 800,
               800); // Open Window
  ...
  mesh.index(&indices);
  mesh.model = glm::translate(glm::mat4(1.0f),
                              glm::vec3(-GRIDSIZE / 2, -15.0, -GRIDSIZE / 2));
  ...
  */
  ...
  Buffer fprop(NX * NY * NZ * Q,
               (float *)NULL);             // Raw FProp Buffer (Collision Step)
  ...
  for (int x = 0; x < NX; x++)
    for (int y = 0; y < NY; y++)
      for (int z = 0; z < NZ; z++) {
        ...
        if (glm::length(glm::vec3(x - NX / 2, y - NY / 2, z - NZ / 2)) < 5.0f)
  ...
  */
  ...
  for (size_t i = 0; i < NPARTICLE; i++) {
    glm::vec4 p = glm::vec4(u(generator), u(generator), u(generator), 1) *
                  glm::vec4(NX, NY, NZ, 1);
  ...
  Shader streamshader(
      {"shader/stream.vs", "shader/stream.gs", "shader/stream.fs"},
      {"in_Position", "in_Direction"});
  ...
  Tiny::view.pipeline = [&]() {      // Setup Drawing Pipeline
    ...
    //	defaultShader.use();
    ////Prepare Shader
    //	defaultShader.uniform("model", mesh.model);			//Set
    // Model Matrix 	defaultShader.uniform("vp", cam::vp);
    ////View Projection Matrix 	mesh.render(GL_LINES);
    ////Render Model with Lines
  ...
  Tiny::loop([&]() { // Autorotate Camera
    ...
    for (auto &p : pos) {
      ...
      if (p.x >= (3 * NX / 4 - 1.0) || p.y >= (3 * NY / 4 - 1.0) ||
          p.z >= (3 * NZ / 4 - 1.0) || p.x <= NX / 4 || p.y <= NY / 4 ||
          p.z <= NZ / 4
          //  || heightmap[(int)p.x][(int)p.z] >= (float)p.y/(float)(NY-1)
          || u(generator) > 0.995) {
        p = glm::vec4(u(generator), u(generator), u(generator), 1) *
            glm::vec4(NX, NY, NZ, 1);
