...
struct cpp_main_caller {
  cpp_main_caller(int (*cpp_main_func)(int argc, char *argv[]), int argc,
                  char **argv)
      : m_cpp_main_func(cpp_main_func), m_argc(argc), m_argv(argv) {}
...
int BOOST_TEST_DECL prg_exec_monitor_main(int (*cpp_main)(int argc,
                                                          char *argv[]),
                                          int argc, char *argv[]) {
  int result = 0;
  ...
  BOOST_TEST_I_TRY {
    boost::unit_test::const_string p(
        std::getenv("BOOST_TEST_CATCH_SYSTEM_ERRORS"));
