...
static void usage(void) {
  ...
  printf("Options:\n");
  printf("      --platform=PLATFORM   the platform to use (" PLATFORM_NAME_ANY
         " or " PLATFORM_NAME_WIN32 " or " PLATFORM_NAME_COCOA
         " or " PLATFORM_NAME_X11 " or " PLATFORM_NAME_WL
         " or " PLATFORM_NAME_NULL ")\n");
  ...
  printf("  -l, --list-extensions     list all Vulkan and client API "
         "extensions\n");
  ...
  printf("      --no-error            request a context that does not emit "
         "errors\n");
  printf("      --angle-type=TYPE     the ANGLE platform type to use "
         "(" ANGLE_TYPE_OPENGL ", " ANGLE_TYPE_OPENGLES ", " ANGLE_TYPE_D3D9
         ", " ANGLE_TYPE_D3D11 ", " ANGLE_TYPE_VULKAN " or " ANGLE_TYPE_METAL
         ")\n");
...
static void list_vulkan_instance_layers(void) {
  ...
  for (uint32_t i = 0; i < lp_count; i++) {
    printf(" %s (spec version %u.%u) \"%s\"\n", lp[i].layerName,
           ...
           VK_VERSION_MINOR(lp[i].specVersion), lp[i].description);
  }
...
static void print_platform(void) {
  const int platforms[] = {GLFW_PLATFORM_WIN32, GLFW_PLATFORM_COCOA,
...
int main(int argc, char **argv) {
  ...
  if (window) {
    ...
    printf("%s context version parsed by GLFW: %u.%u.%u\n",
           get_api_name(client), major, minor, revision);
    ...
    if (client == GLFW_OPENGL_API && profile != GLFW_OPENGL_CORE_PROFILE) {
      ...
      printf(" accum red: %u accum green: %u accum blue: %u accum alpha: %u "
             "aux buffers: %u\n",
