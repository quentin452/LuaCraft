...
function getBlock(pointer, x, y, z)
	...
	if x >= size then
		return n1p and getBlock(n1p, x % size, y % size, z % size) or -1
	end
	if x < 0 then
		return n2p and getBlock(n2p, x % size, y % size, z % size) or -1
	end
	if y >= size then
		return n3p and getBlock(n3p, x % size, y % size, z % size) or -1
	end
	if y < 0 then
		return n4p and getBlock(n4p, x % size, y % size, z % size) or -1
	end
	if z >= size then
		return n5p and getBlock(n5p, x % size, y % size, z % size) or -1
	end
	if z < 0 then
	...
	for y = 0, size - 1 do
		for z = 0, size - 1 do
			if getBlock(blockdatapointer, x, y, z) ~= 0 then
				if getBlock(blockdatapointer, x + 1, y, z) == 0 then
					facecount = facecount + 1
				end
				if getBlock(blockdatapointer, x - 1, y, z) == 0 then
					facecount = facecount + 1
				end
				if getBlock(blockdatapointer, x, y + 1, z) == 0 then
					facecount = facecount + 1
				end
				if getBlock(blockdatapointer, x, y - 1, z) == 0 then
					facecount = facecount + 1
				end
				if getBlock(blockdatapointer, x, y, z + 1) == 0 then
					facecount = facecount + 1
				end
				if getBlock(blockdatapointer, x, y, z - 1) == 0 then
					facecount = facecount + 1
	...
	for x = 0, size - 1 do
		for y = 0, size - 1 do
			for z = 0, size - 1 do
				if getBlock(blockdatapointer, x, y, z) ~= 0 then
					if getBlock(blockdatapointer, x - 1, y, z) == 0 then
						addFace(x, y, z, 0, 1, 2, u1, v1, c2)
					end
					if getBlock(blockdatapointer, x + 1, y, z) == 0 then
						addFace(x + 1, y, z, 0, 1, 2, u1, v1, c2)
					end
					if getBlock(blockdatapointer, x, y - 1, z) == 0 then
						addFace(x, y, z, 1, 0, 2, u1, v1, c1)
					end
					if getBlock(blockdatapointer, x, y + 1, z) == 0 then
						addFace(x, y + 1, z, 1, 0, 2, u1, v1, c1)
					end
					if getBlock(blockdatapointer, x, y, z - 1) == 0 then
						addFace(x, y, z, 1, 2, 0, u1, v1, c3)
					end
					if getBlock(blockdatapointer, x, y, z + 1) == 0 then
						addFace(x, y, z + 1, 1, 2, 0, u2, v2, c1)
					end
				end
			end
		end
