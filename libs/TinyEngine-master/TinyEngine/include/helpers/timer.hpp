...
template <typename D> struct measure {
...
public:
  // Timeout Setter
  template <typename F> void set_timeout(D duration, F function) {
    active = true;
    t = std::thread([&]() {
      std::this_thread::sleep_for(duration);
      if (!active)
        return; // Maybe the Thread was terminated before the execution of
                // function
      function();
      return;
    });
  ...
  void set_const_interval(D *duration, F function, Args &&...args) {
    ...
    t = std::thread([&]() {
      while (active) {
        ...
        function(args...);
        delayTime = std::chrono::duration_cast<D>(
