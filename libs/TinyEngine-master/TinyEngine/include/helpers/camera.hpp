...
vec3 pos = vec3(cos(glm::radians(roty)) * sin(glm::radians(rot)),
                sin(glm::radians(roty)),
                -cos(glm::radians(roty)) * cos(glm::radians(rot)));
...
void update() {
  //    vec3 pos = vec3(sin(glm::radians(rot)), sin(glm::radians(roty)),
  //    cos(glm::radians(rot)));

  pos = vec3(cos(glm::radians(roty)) * sin(glm::radians(rot)),
             sin(glm::radians(roty)),
             -cos(glm::radians(roty)) * cos(glm::radians(rot)));
  ...
  if (type == ORTHO)
    view = lookAt(look + pos, look, vec3(0, 1, 0));
...
void init(float r = 5.0f, camtype t = PROJ) {
  ...
  if (type == PROJ)
    proj = glm::perspective(
        FOV, (float)Tiny::view.WIDTH / (float)Tiny::view.HEIGHT, near, far);
  if (type == ORTHO)
    proj = glm::ortho(-(float)Tiny::view.WIDTH / rad,
                      (float)Tiny::view.WIDTH / rad,
                      -(float)Tiny::view.HEIGHT / rad,
                      (float)Tiny::view.HEIGHT / rad, near, far);
