...
private:
  ...
  std::unordered_map<std::string, int> instances; // Binding Points of
                                                  // Attributes
...
template <typename T> void Instance::bind(std::string name, Buffer *buf) {
  SIZE = buf->SIZE;
...
template <typename T> void Instance::config(Buffer *buf) {
  glEnableVertexAttribArray(model->bindings.size() + instances.size());
  glVertexAttribPointer(model->bindings.size() + instances.size(),
                        sizeof(T) / sizeof(GLfloat), GL_FLOAT, GL_FALSE, 0,
                        (void *)0);
...
void Instance::render(GLenum mode, int size) {
  ...
  if (model->indexed) {
    ...
    glDrawArraysInstanced(mode, 0, model->SIZE, size);
}
...
void Instance::config<glm::mat4>(Buffer *buf) {
  for (int i = 0; i < 4; i++) {
    ...
    glVertexAttribPointer(model->bindings.size() + instances.size() + i, 4,
                          GL_FLOAT, GL_FALSE, 4 * sizeof(glm::vec4),
                          (void *)(i * sizeof(glm::vec4)));
    glVertexAttribDivisor(model->bindings.size() + instances.size() + i, 1);
  }
}

void Instance::render(GLenum mode) { render(mode, SIZE); }
