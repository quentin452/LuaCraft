...
function ProFi:checkMemory(interval, note)
	...
	if not self.memoryReports then
		self.memoryReports = {}
	end
...
function ProFi:writeProfilingReportsToFile(reports, file)
	local totalTime = self.stopTime and self.startTime and (self.stopTime - self.startTime) or 0
	local totalTimeOutput = string.format(FORMAT_TOTALTIME_LINE, totalTime or 0)
	file:write(totalTimeOutput)
	local header = string.format(FORMAT_HEADER_LINE, "FILE", "FUNCTION", "LINE", "TIME", "RELATIVE", "CALLED")
	file:write(header)
	for i, funcReport in ipairs(reports or {}) do
		local timer = string.format(FORMAT_TIME, funcReport.timer or 0)
		local count = string.format(FORMAT_COUNT, funcReport.count or 0)
		local relTime = string.format(
			FORMAT_RELATIVE,
			(funcReport.timer and totalTime and (funcReport.timer / totalTime) * 100) or 0
		)
		local outputLine = string.format(FORMAT_OUTPUT_LINE, funcReport.title or "", timer, relTime, count)
