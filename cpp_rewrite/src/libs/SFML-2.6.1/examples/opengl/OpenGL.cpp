...
#include <src/libs/SFML-2.6.1/include/SFML/Main.hpp>
#endif
...
int main() {
  ...
  while (!exit) {
    ...
    glLoadIdentity();
    GLfloat ratio = static_cast<float>(window.getSize().x) /
    ...
    while (window.isOpen()) {
      ...
      while (window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::Resized) {
          ...
          glMatrixMode(GL_PROJECTION);
          glLoadIdentity();
          GLfloat newRatio = static_cast<float>(event.size.width) /
      ...
      // We get the position of the mouse cursor (or touch), so that we can move
      // the box accordingly
      sf::Vector2i pos;
      ...
      float x = static_cast<float>(pos.x) * 200.f /
                    static_cast<float>(window.getSize().x) -
                100.f;
      float y = -static_cast<float>(pos.y) * 200.f /
                    static_cast<float>(window.getSize().y) +
                100.f;
