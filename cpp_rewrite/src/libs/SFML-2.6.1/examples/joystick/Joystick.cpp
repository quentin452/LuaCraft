...
int main() {
  ...
  sf::RenderWindow window(sf::VideoMode(400, 775), "Joystick",
                          sf::Style::Close);
  ...
  for (unsigned int i = 0; i < sf::Joystick::AxisCount; ++i) {
    ...
    object.label.setPosition(
        5.f, 5.f + (static_cast<float>(i + 4) * font.getLineSpacing(14)));
    object.label.setString(std::string(axislabels[i]) + ":");

    object.value.setPosition(
        80.f, 5.f + (static_cast<float>(i + 4) * font.getLineSpacing(14)));
  ...
  for (unsigned int i = 0; i < sf::Joystick::ButtonCount; ++i) {
    ...
    object.label.setPosition(
        5.f, 5.f + (static_cast<float>(sf::Joystick::AxisCount + i + 4) *
                    font.getLineSpacing(14)));
    object.label.setString(sstr.str() + ":");

    object.value.setPosition(
        80.f, 5.f + (static_cast<float>(sf::Joystick::AxisCount + i + 4) *
                     font.getLineSpacing(14)));
    object.value.setString("N/A");
  }
