...
// Headers
////////////////////////////////////////////////////////////
#include <src/libs/SFML-2.6.1/include/SFML/Config.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Window/Joystick.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Window/Keyboard.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Window/Mouse.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Window/Sensor.hpp>
...
public:
  ...
  struct KeyEvent {
    ...
    Keyboard::Scancode
        scancode; //!< Physical code of the key that has been pressed
  ...
  struct MouseWheelEvent {
    int delta; //!< Number of ticks the wheel has moved (positive is up,
               //!< negative is down)
    int x;     //!< X position of the mouse pointer, relative to the left of the
               //!< owner window
    int y;     //!< Y position of the mouse pointer, relative to the top of the
  ...
  struct MouseWheelScrollEvent {
    ...
    float
        delta; //!< Wheel offset (positive is up/left, negative is down/right).
  ...
  struct JoystickConnectEvent {
    unsigned int joystickId; //!< Index of the joystick (in range [0 ..
                             //!< Joystick::Count - 1])
  };
  ...
  struct JoystickButtonEvent {
    unsigned int joystickId; //!< Index of the joystick (in range [0 ..
                             //!< Joystick::Count - 1])
    unsigned int button;     //!< Index of the button that has been pressed (in
                             //!< range [0 .. Joystick::ButtonCount - 1])
  };
  ...
  struct TouchEvent {
    ...
    int x; //!< X position of the touch, relative to the left of the owner
           //!< window
  ...
  enum EventType {
    ...
    MouseWheelMoved,     //!< The mouse wheel was scrolled (data in
                         //!< event.mouseWheel) (deprecated)
    MouseWheelScrolled,  //!< The mouse wheel was scrolled (data in
                         //!< event.mouseWheelScroll)
    MouseButtonPressed,  //!< A mouse button was pressed (data in
                         //!< event.mouseButton)
    MouseButtonReleased, //!< A mouse button was released (data in
                         //!< event.mouseButton)
    ...
    JoystickButtonReleased, //!< A joystick button was released (data in
                            //!< event.joystickButton)
    JoystickMoved,          //!< The joystick moved along an axis (data in
                            //!< event.joystickMove)
    JoystickConnected,      //!< A joystick was connected (data in
                            //!< event.joystickConnect)
    JoystickDisconnected,   //!< A joystick was disconnected (data in
                            //!< event.joystickConnect)
  ...
  union {
    ...
    KeyEvent
        key; //!< Key event parameters (Event::KeyPressed, Event::KeyReleased)
    TextEvent text; //!< Text event parameters (Event::TextEntered)
    MouseMoveEvent
        mouseMove; //!< Mouse move event parameters (Event::MouseMoved)
    ...
    MouseWheelEvent mouseWheel;   //!< Mouse wheel event parameters
                                  //!< (Event::MouseWheelMoved) (deprecated)
    ...
    JoystickConnectEvent
        joystickConnect; //!< Joystick (dis)connect event parameters
                         //!< (Event::JoystickConnected,
                         //!< Event::JoystickDisconnected)
    TouchEvent touch;    //!< Touch events parameters (Event::TouchBegan,
                         //!< Event::TouchMoved, Event::TouchEnded)
