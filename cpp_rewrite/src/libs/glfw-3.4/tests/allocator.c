...
static void deallocate(void *block, void *user) {
  ...
  printf("%s: deallocate %zu bytes (current %zu maximum %zu total %zu)\n",
         function_name, *real_block, stats->current, stats->maximum,
         stats->total);
...
static void *reallocate(void *block, size_t size, void *user) {
  ...
  printf("%s: reallocate %zu bytes to %zu bytes (current %zu maximum %zu total "
         "%zu)\n",
         function_name, *real_block, size, stats->current, stats->maximum,
         stats->total);
...
int main(void) {
  ...
  const GLFWallocator allocator = {.allocate = allocate,
  ...
  GLFWwindow *window =
      CALL(glfwCreateWindow)(400, 400, "Custom allocator test", NULL, NULL);
  if (!window) {
    glfwTerminate();
    exit(EXIT_FAILURE);
  }
