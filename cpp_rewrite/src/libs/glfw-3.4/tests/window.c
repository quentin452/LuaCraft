...
int main(int argc, char **argv) {
  ...
  GLFWwindow *window =
      glfwCreateWindow(600, 660, "Window Features", NULL, NULL);
  if (!window) {
    glfwTerminate();
    exit(EXIT_FAILURE);
  }
    ...
    if (nk_begin(nk, "main", area, 0)) {
      ...
      if (glfwGetWindowMonitor(window)) {
        if (nk_button_label(nk, "Make Windowed")) {
          glfwSetWindowMonitor(window, NULL, windowed_x, windowed_y,
      ...
      nk_label(nk, "Press Enter in a text field to set value",
               NK_TEXT_CENTERED);
      ...
      if (position_supported) {
        ...
        events = nk_edit_string_zero_terminated(
            nk, flags, xpos_buffer, sizeof(xpos_buffer), nk_filter_decimal);
        if (events & NK_EDIT_COMMITED) {
        ...
        events = nk_edit_string_zero_terminated(
            nk, flags, ypos_buffer, sizeof(ypos_buffer), nk_filter_decimal);
        if (events & NK_EDIT_COMMITED) {
      ...
      events = nk_edit_string_zero_terminated(
          nk, flags, width_buffer, sizeof(width_buffer), nk_filter_decimal);
      if (events & NK_EDIT_COMMITED) {
      ...
      events = nk_edit_string_zero_terminated(
          nk, flags, height_buffer, sizeof(height_buffer), nk_filter_decimal);
      if (events & NK_EDIT_COMMITED) {
      ...
      events = nk_edit_string_zero_terminated(
          nk, flags, numer_buffer, sizeof(numer_buffer), nk_filter_decimal);
      if (events & NK_EDIT_COMMITED) {
      ...
      events = nk_edit_string_zero_terminated(
          nk, flags, denom_buffer, sizeof(denom_buffer), nk_filter_decimal);
      if (events & NK_EDIT_COMMITED) {
      ...
      nk_value_bool(nk, "Iconified",
                    glfwGetWindowAttrib(window, GLFW_ICONIFIED));
      nk_value_bool(nk, "Maximized",
