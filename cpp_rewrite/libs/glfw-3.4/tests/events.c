...
static void key_callback(GLFWwindow *window, int key, int scancode, int action,
  ...
  if (key == GLFW_KEY_UNKNOWN) {
    printf("%08x to %i at %0.3f: Key (%s) Scancode 0x%04x (with%s) was %s\n",
           counter++, slot->number, glfwGetTime(), get_key_name(key), scancode,
           get_mods_name(mods), get_action_name(action));
  } else {
    const char *name = glfwGetKeyName(key, scancode);
    if (name) {
      printf("%08x to %i at %0.3f: Key 0x%04x (%s) Scancode 0x%04x Name %s "
             "(with%s) was %s\n",
             counter++, slot->number, glfwGetTime(), key, get_key_name(key),
             scancode, name, get_mods_name(mods), get_action_name(action));
    } else {
      printf("%08x to %i at %0.3f: Key 0x%04x (%s) Scancode 0x%04x (with%s) "
             "was %s\n",
             counter++, slot->number, glfwGetTime(), key, get_key_name(key),
             scancode, get_mods_name(mods), get_action_name(action));
    }
  }
...
static void joystick_callback(int jid, int event) {
  if (event == GLFW_CONNECTED) {
    ...
    printf("%08x at %0.3f: Joystick %i (%s) was connected with %i axes, %i "
           "buttons, and %i hats\n",
    ...
    if (glfwJoystickIsGamepad(jid)) {
      printf("  Joystick %i (%s) has a gamepad mapping (%s)\n", jid,
             glfwGetJoystickGUID(jid), glfwGetGamepadName(jid));
    } else {
