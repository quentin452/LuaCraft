...
////////////////////////////////////////////////////////////
#include <map>
#include <src/libs/SFML-2.6.1/include/SFML/Network/Export.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Network/IpAddress.hpp>
#include <src/libs/SFML-2.6.1/include/SFML/Network/TcpSocket.hpp>
...
public:
  ...
  public:
    ...
    enum Status {
      ...
      Accepted = 202,  //!< The request has been accepted, but will be processed
                       //!< later by the server
      NoContent = 204, //!< The server didn't send any data in return
      ResetContent =
          205, //!< The server informs the client that it should clear the view
               //!< (form) that caused the request to be sent
      PartialContent = 206, //!< The server has sent a part of the resource, as
                            //!< a response to a partial GET request
      ...
      NotModified = 304, //!< For conditional requests, means the requested page
      ...
      Forbidden = 403, //!< The requested page cannot be accessed at all, even
                       //!< with authentication
      NotFound = 404,  //!< The requested page doesn't exist
      RangeNotSatisfiable = 407, //!< The server can't satisfy the partial GET
                                 //!< request (with a "Range" header field)

      // 5xx: server error
      InternalServerError = 500, //!< The server encountered an unexpected error
      NotImplemented =
          501,          //!< The server doesn't implement a requested feature
      BadGateway = 502, //!< The gateway server has received an error from the
                        //!< source server
      ServiceNotAvailable = 503, //!< The server is temporarily unavailable
                                 //!< (overloaded, in maintenance, ...)
      GatewayTimeout = 504, //!< The gateway server couldn't receive a response
