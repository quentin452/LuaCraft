...
sf::Color getHighlandsTerrainColor(float elevation, float moisture) {
  ...
  color.r = static_cast<sf::Uint8>(lowlandsColor.r * (1.f - factor) +
                                   color.r * factor);
  color.g = static_cast<sf::Uint8>(lowlandsColor.g * (1.f - factor) +
                                   color.g * factor);
  color.b = static_cast<sf::Uint8>(lowlandsColor.b * (1.f - factor) +
                                   color.b * factor);
...
sf::Color getSnowcapTerrainColor(float elevation, float moisture) {
  ...
  color.r = static_cast<sf::Uint8>(highlandsColor.r * (1.f - factor) +
                                   color.r * factor);
  color.g = static_cast<sf::Uint8>(highlandsColor.g * (1.f - factor) +
                                   color.g * factor);
  color.b = static_cast<sf::Uint8>(highlandsColor.b * (1.f - factor) +
                                   color.b * factor);
...
sf::Color getTerrainColor(float elevation, float moisture) {
  sf::Color color =
      elevation < 0.11f
          ? sf::Color(0, 0,
                      static_cast<sf::Uint8>(elevation / 0.11f * 74.f + 181.0f))
      : elevation < 0.14f
    ...
    for (unsigned int x = 0; x < resolutionX; x++) {
      ...
      if (x > 0) {
        ...
        vertices[arrayIndexBase + 0].position =
            sf::Vector2f(static_cast<float>(x) * scalingFactorX,
        ...
        vertices[arrayIndexBase + 0].texCoords =
            computeNormal(getElevation(x - 1, y), getElevation(x + 1, y),
      ...
      vertices[arrayIndexBase + 2].position =
          sf::Vector2f(static_cast<float>(x + 1) * scalingFactorX,
                       static_cast<float>(y + 1) * scalingFactorY);
      ...
      vertices[arrayIndexBase + 2].texCoords =
          computeNormal(getElevation(x, y + 1), getElevation(x + 2, y + 1),
      ...
      if (y > rowStart) {
        ...
        vertices[arrayIndexBase + 5].texCoords = computeNormal(
            getElevation(x, y), getElevation(x + 2, y),
            getElevation(x + 1, y + 1), getElevation(x + 1, y - 1));
      }
    }
  }
