BlockModellingChannel, ChunkSize, ChunkHashTable ,WorldHeight= ...
...
function BlockRendering(chunk, i, j, k, x, y, z, thisTransparency, thisLight, model, BlockModelScale)
...
while true do
	local data = BlockModellingChannel:demand() -- Attend les données
	if data then
		local i, j, k, x, y, z, thisTransparency, Light, SliceModels, BlockModelScale, chunkX, chunkY, chunkZ =
			unpack(data)
		local chunk = GetChunk(chunkX, chunkY, chunkZ) -- Remplacez ceci avec votre propre logique pour obtenir le chunk
