...
//  [X] Renderer: User texture binding. Use 'MTLTexture' as ImTextureID. Read
...
//  2022-08-23: Metal: Update deprecated property
...
//  Metal: Ignore ImDrawCmd where ElemCount == 0 (very rare but can technically
...
//  2021-08-24: Metal: Fixed a crash when clipping rect larger than framebuffer
//  is submitted. (#4464) 2021-05-19: Metal: Replaced direct access to
...
//  2019-04-30: Metal: Added support for special ImDrawCallback_ResetRenderState
...
static ImGui_ImplMetal_Data *ImGui_ImplMetal_CreateBackendData() {
...
void ImGui_ImplMetal_NewFrame(MTLRenderPassDescriptor *renderPassDescriptor) {
  ImGui_ImplMetal_Data *bd = ImGui_ImplMetal_GetBackendData();
  IM_ASSERT(bd->SharedMetalContext != nil &&
            "No Metal context. Did you call ImGui_ImplMetal_Init() ?");
...
static void ImGui_ImplMetal_SetupRenderState(
    ImDrawData *drawData, id<MTLCommandBuffer> commandBuffer,
    id<MTLRenderCommandEncoder> commandEncoder,
    id<MTLRenderPipelineState> renderPipelineState, MetalBuffer *vertexBuffer,
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback) {
        ...
        if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
          ImGui_ImplMetal_SetupRenderState(drawData, commandBuffer,
                                           commandEncoder, renderPipelineState,
        ...
        [commandEncoder
            setVertexBufferOffset:(vertexBufferOffset +
                                   pcmd->VtxOffset * sizeof(ImDrawVert))
                          atIndex:0];
        [commandEncoder
...
bool ImGui_ImplMetal_CreateFontsTexture(id<MTLDevice> device) {
  ...
  // ImTextureID facility to render users own textures. You can make that change
