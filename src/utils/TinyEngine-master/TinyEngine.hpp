...
void sighandler(int signal) { event.quit = true; }
...
bool init() {
  ...
  if (!view.windowed &&
      !view.init("TinyEngine OpenGL Context", 0, 0)) { // Open a Dummy Window
...
template <typename F, typename... Args> void loop(F function, Args &&...args) {
  ...
  if (!benchmark)
    ...
    while (!event.quit) {
      ...
      if (Tiny::view.enabled) {
        ...
        average = 0.99 * average +
                  0.01 * timer::benchmark<std::chrono::microseconds>([&]() {
                    view.render(); // Render View
                    glFinish();
                  });
      }
