...
function perlin:noiseV1(x, y, z)
	y = y or 0
	z = z or 0

	-- Calculate the "unit cube" that the point asked will be located in
	local xi = self:band(math.floor(x), 255)
	local yi = self:band(math.floor(y), 255)
	local zi = self:band(math.floor(z), 255)
	...
	-- Hash all 8 unit cube coordinates surrounding the input coordinate
	local p = self.p
...
function perlin:grad(hash, x, y, z)
	return self.dot_product[self:band(hash, 0xF)](x, y, z)
end
