...
function NewSunlightUpDownOperation(x, y, z, value, operation)
	_JPROFILER.push("NewSunlightUpDownOperation")
	...
	while #queue > 0 do
		...
		if cget then
			...
			if operation == "addition" then
				if TileLightable(val) and dat <= item.value then
					_JPROFILER.push("NewSunlightUpDownOperation_addition")
					cget:setVoxelFirstData(cx, cy, cz, item.value)
					_JPROFILER.pop("NewSunlightUpDownOperation_addition")
					table.insert(queue, { x = item.x, y = item.y - 1, z = item.z, value = item.value })
					for _, dir in ipairs(FOURDIRECTIONS) do
						_JPROFILER.push("NewSunlightUpDownOperation_addition_loop")
						NewSunlightAddition(item.x + dir.x, item.y + dir.y, item.z + dir.z, item.value - 1)
						_JPROFILER.pop("NewSunlightUpDownOperation_addition_loop")
					end
				end
			elseif operation == "substraction" then
				if TileSemiLightable(val) then
					_JPROFILER.push("NewSunlightUpDownOperation_subtraction")
					SetVoxelFirstData(item.x, item.y, item.z, Tiles.AIR_Block.id)
					_JPROFILER.pop("NewSunlightUpDownOperation_subtraction")
					for _, dir in ipairs(FOURDIRECTIONS) do
						local nx, ny, nz = item.x + dir.x, item.y + dir.y, item.z + dir.z
						_JPROFILER.push("NewSunlightUpDownOperation_subtraction_loop")
						NewSunlightSubtraction(nx, ny, nz, LightSources[15])
						_JPROFILER.pop("NewSunlightUpDownOperation_subtraction_loop")
					end
				end
			end
		end
	end
	_JPROFILER.pop("NewSunlightUpDownOperation")
end
