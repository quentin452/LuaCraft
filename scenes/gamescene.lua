...
blockCursor.texture = texturepack
end
...
function GameScene:requestRemesh(chunk, first)
    ...
    if not self.chunkMap[key1] then return end
    if not self.chunkMap[key2] then return end
    if not self.chunkMap[key3] then return end
    if not self.chunkMap[key4] then return end
    if not self.chunkMap[key5] then return end
    if not self.chunkMap[key6] then return end
