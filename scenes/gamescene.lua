		...
		if chunk then
			...
			if chunk:getBlock(lx, ly, lz) ~= 0 then
				...
				if leftClick then
					...
					if lx >= size - 1 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x + 1, y, z)], true)
					end
					if lx <= 0 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x - 1, y, z)], true)
					end
					if ly >= size - 1 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x, y + 1, z)], true)
					end
					if ly <= 0 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x, y - 1, z)], true)
					end
					if lz >= size - 1 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x, y, z + 1)], true)
					end
					if lz <= 0 then
						self:requestRemesh(self.chunkMap[("%d/%d/%d"):format(x, y, z - 1)], true)
					end
